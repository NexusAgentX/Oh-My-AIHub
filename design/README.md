...
| 资产 | 内容 | 评审用途 |
| --- | --- | --- |
| `sources/core-workflows.op` | 49 个可点击桌面页面，以及流程、状态和响应式说明 | 在 OpenPencil 中编辑、跳转和检查完整交互 |
| `previews/core-workflows.png` | API 市场、消费者、共享者、C2C 与管理员五块评审总览 | 在 GitHub Issue 和 Pull Request 中快速确认方向 |

可点击页面覆盖受邀登录、管理员一次性初始凭据交付、首次及日常改密，API 市场比较、公开渠道详情、用户评分、加入模型池及无匹配、暂停、重复或不兼容状态，平台 Key 的创建、轮换和撤销，渠道凭据校验与替换，模型协议池、顺序渠道回退、调用成功、平台预校验、全部失败与流式中断，C2C 买卖挂单、部分成交、买家等待、卖家放行、取消、超时与用户争议，以及管理员账户与信用额度、模型目录、争议裁决、共享者收入和运营指标。普通产品与管理后台使用独立导航。源文件中的名称、地址、金额、凭据和指标均为不可用的原型示例，不是生产数据。
