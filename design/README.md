...
- 当前的 `core-workflows` 是首版产品交互与公开叙事基线：源文件包含可点击原型、四角色流程图、评审总览、响应式说明和 SaaS 落地页五页；落地页同时提供完整桌面布局和独立重排的移动布局，只引导受邀用户登录，不代表开放注册。同名 PNG 是从“评审总览”页导出的 Git 评审入口。
...
| 资产 | 内容 | 评审用途 |
| --- | --- | --- |
| `sources/core-workflows.op` | 48 个可点击桌面页面、流程、状态、响应式说明，以及 Airtable 视觉语言下的桌面与移动公开落地页 | 在 OpenPencil 中编辑、跳转和检查完整交互与产品叙事 |
| `previews/core-workflows.png` | API 市场、消费者、共享者、C2C、管理员与公开落地页评审总览 | 在 GitHub Issue 和 Pull Request 中快速确认方向 |
