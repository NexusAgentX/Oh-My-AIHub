...
产品方向和首版边界已经确认。当前代码已经交付公开产品落地页、受邀账户、首次改密、管理员账户管理、公开模型目录，以及中心化零和账本、钱包与管理员账本运营视图；渠道共享、平台代理结算与 C2C 仍按 Epic #16 继续实现。已确认需求见 `PRODUCT.md`，推进顺序见 `ROADMAP.md`，当前真实实现见 `ARCHITECTURE.md`。
...
- 后端：Go HTTP 服务、受邀账户、模型目录、零和账本、钱包与管理员账本 API。
- 数据库：PostgreSQL 18，使用 Goose 管理嵌入式 SQL 迁移。
- 本地工具链：mise。
- 容器运行：Docker Compose，前端由 Nginx 提供静态资源并代理 `/api` 请求，迁移完成后再启动后端。

当前可运行能力包括未认证可访问的 `/welcome` 产品落地页、用户名登录、首次强制改密、账户设置、管理员创建与停用账户、管理员权限、信用额度与信用冻结维护，以及公开模型目录的新增、查询、编辑与启停。已改密用户可以查看真实钱包、可消费额度和可追溯分录；余额不足页提供 C2C 恢复入口；管理员可以查看全局零和与投影一致性、信用风险、激励/损失账户和单账户明细，并执行有对手方的调整与坏账转移。落地页所有账户 CTA 都进入真实 `/login`；API 渠道、平台 Key、代理调用和 C2C 业务流程尚未交付。
