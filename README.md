...
产品方向和首版边界已经确认。当前代码已经交付公开产品落地页、受邀账户、首次改密、管理员账户管理和公开模型目录；API 共享、积分账本、路由结算与 C2C 仍按 Epic #16 继续实现。已确认需求见 `PRODUCT.md`，推进顺序见 `ROADMAP.md`，当前真实实现见 `ARCHITECTURE.md`。
...
当前可运行能力包括未认证可访问的 `/welcome` 产品落地页、用户名登录、首次强制改密、账户设置、管理员创建与停用账户、管理员权限与信用额度维护，以及公开模型目录的新增、查询、编辑与启停。落地页所有账户 CTA 都进入真实 `/login`；余额、API 渠道、平台 Key、代理调用、结算和 C2C 尚未交付，接口中的余额字段只是后续账本接入前的临时投影。
