    ...
    <AppShell>
      ...
      <section className="panel table-panel market-channel-offers">
        ...
        {channel.offers.length === 0 ? <div className="empty-state">渠道当前不可用</div> : <>
          <div className="desktop-table-wrap"><table className="data-table"><thead><tr><th scope="col">模型</th><th scope="col">API 格式</th><th scope="col">倍率</th><th scope="col">输入 / 输出</th><th scope="col">缓存写 / 读</th><th scope="col">质量</th></tr></thead><tbody>{channel.offers.map((offer) => <tr key={offer.offer_id}>
            <td><strong>{offer.model_name}</strong><small>{offer.model_provider}</small></td><td>{protocolLabels[offer.protocol]}</td><td>{offer.multiplier}×</td><td><PricePair first={offer.input_price} second={offer.output_price} /></td><td><PricePair first={offer.cache_write_price} second={offer.cache_read_price} /></td><td>{offer.call_success_rate === null ? <><span className="quality-empty">暂无调用数据</span><small>验证 {formatDate(offer.last_tested_at)}</small></> : `${offer.call_success_rate} · ${offer.ttft_milliseconds ?? '—'} ms`}</td>
          </tr>)}</tbody></table></div>
          <div className="mobile-card-list">{channel.offers.map((offer) => <article className="mobile-data-card" key={offer.offer_id}><header><div><strong>{offer.model_name}</strong><span>{offer.model_provider}</span></div><ChannelStateBadge status={offer.validation_status} /></header><dl><div><dt>API 格式</dt><dd>{protocolLabels[offer.protocol]}</dd></div><div><dt>倍率</dt><dd>{offer.multiplier}×</dd></div><div><dt>输入 / 输出</dt><dd><PricePair first={offer.input_price} second={offer.output_price} /></dd></div><div><dt>缓存写 / 读</dt><dd><PricePair first={offer.cache_write_price} second={offer.cache_read_price} /></dd></div><div><dt>质量</dt><dd>{offer.call_success_rate ?? '暂无调用数据'}</dd></div></dl></article>)}</div>
        </>}
      </section>
    </AppShell>
  )
