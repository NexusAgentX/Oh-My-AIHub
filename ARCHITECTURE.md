...
> 状态：公开产品落地页、受邀账户、模型目录与零和账本基础已实现；渠道、平台代理结算与 C2C 继续按 Epic #16 交付
...
| 组件 | 位置 | 当前职责 |
| --- | --- | --- |
| 前端 | `frontend/` | React 单页应用；提供未认证的 `/welcome` 产品落地页，以及登录、首次改密、账户设置、钱包/分录、管理员账户、模型目录与账本运营界面 |
| 后端 | `backend/` | Go HTTP 服务；提供健康、身份、账户、模型目录、零和账本、钱包和管理员账本 JSON API |
| 数据库 | PostgreSQL 18 | 持久化账户、会话、模型元数据、不可变账本事实、同步投影与管理员审计事件 |
...
4. 身份、模型目录和账本服务通过 PostgreSQL Store 读写状态；业务调用方可以在同一数据库事务中原子提交账本与业务记录，健康检查也验证数据库可用性。
