...
> 状态：公开落地页、受邀账户、模型目录、零和账本、渠道市场、平台 API Key、四协议代理与调用结算已实现；C2C 继续按 Epic #16 交付
...
| 组件 | 位置 | 当前职责 |
| --- | --- | --- |
| 前端 | `frontend/` | React 单页应用；提供公共落地页、身份、钱包、渠道市场、平台 Key/池配置、调用记录和管理员运营界面 |
| 后端 | `backend/` | Go HTTP 服务；提供管理 JSON API，以及 Chat Completions、Responses、Anthropic Messages 和 Gemini GenerateContent 原生代理入口 |
| 数据库 | PostgreSQL 18 | 持久化账户、会话、模型、不可变账本、渠道、加密凭据、平台 Key 摘要、池、调用快照、尝试、结算、指标与审计事件 |
...
PostgreSQL 是唯一持久化依赖。渠道校验只有在用户明确确认可能产生费用后才访问第三方中转站；消费者使用平台 Key 发起真实调用时，网关会按其模型协议池访问共享者配置的第三方上游。公开市场和普通管理读取不会调用上游。
