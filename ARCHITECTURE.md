...
> 状态：公开产品落地页、受邀账户、模型目录、零和账本、渠道托管与 API 市场已实现；平台代理结算与 C2C 继续按 Epic #16 交付
...
| 组件 | 位置 | 当前职责 |
| --- | --- | --- |
| 前端 | `frontend/` | React 单页应用；提供公共落地页，以及身份、账户、钱包、模型目录、渠道配置、公开市场和管理员运营界面 |
| 后端 | `backend/` | Go HTTP 服务；提供身份、目录、账本、渠道生命周期、校验、市场、评分和渠道治理 JSON API |
| 数据库 | PostgreSQL 18 | 持久化账户、会话、模型、不可变账本、渠道、加密凭据、报价、校验历史、评分、同步投影与审计事件 |
...
4. 身份、模型目录、账本和渠道服务通过 PostgreSQL Store 读写状态；业务调用方可以在同一数据库事务中原子提交账本与业务记录，健康检查也验证数据库可用性。
...
PostgreSQL 是唯一持久化依赖。只有用户明确确认可能产生上游费用后，渠道校验才会请求其配置的第三方中转站；公开市场和普通管理读取不会调用上游。首版代理调用仍未实现。
...
- 渠道调用、平台手续费与 C2C 状态机尚未接入已实现的账本原语；后续 Feature 必须使用事务绑定接口，不得直接更新余额投影。
...
除已经进入上方“当前结构”的 `/welcome`、身份、目录与账本基础外，这些约束将在 Epic #16 的其余独立 Feature 中交付。只有实现合并并通过相应验收后，才会进入上方“当前结构”。
